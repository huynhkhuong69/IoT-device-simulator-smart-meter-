...
class ElectricMeter(object):
    def __init__(self, client: KaaClient):
        ...
        self.config = {"overdriveThreshold": 240, "powerThreshold": None}
    ...
    def _is_overdrive(self, voltage_total: int) -> bool:
        return bool(self.config["overdriveThreshold"]
                    and voltage_total > self.config["overdriveThreshold"] * 3)
    ...
    def get_data_sample(self):
        ...
        for line in self.lines:
            ...
            data[f"current_{line}_reactive"] = self.get_current_reactive(
                data[f"current_{line}_active"])
            data[f"current_{line}_total"] = data[
                f"current_{line}_active"] + data[f"current_{line}_reactive"]
            data[f"power_{line}_active"] = self.get_power(
                data[f"voltage_{line}"], data[f"current_{line}_active"])
            data[f"power_{line}_reactive"] = self.get_power(
                data[f"voltage_{line}"], data[f"current_{line}_reactive"])
            data[f"power_{line}_total"] = self.get_power(
                data[f"voltage_{line}"], data[f"current_{line}_total"])
            data[f"status_{line}"] = getattr(self, line)

        data["current_total"] = sum(
            [data[f"current_{line}_total"] for line in self.lines])
        data["power_total"] = sum(
            [data[f"power_{line}_total"] for line in self.lines])
        data["power_active"] = sum(
            [data[f"power_{line}_active"] for line in self.lines])
        data["power_reactive"] = sum(
            [data[f"power_{line}_reactive"] for line in self.lines])
    ...
    def electric_meter_handler(self, client: mqtt.Client, userdata,
                               message: MQTTMessage):
        ...
        client.publish(response_topic,
                       self.kaa_client.compose_commands_result(commands))
    ...
    def electric_meter_overdrive_handler(self, client: mqtt.Client, userdata,
                                         message: MQTTMessage):
        command_payload = str(message.payload.decode('utf-8'))
        logging.info(f"Received command: [{message.topic}] []")
        response_topic = KaaClient.get_command_response_topic(
            message, "electric_meter_overdrive")
        commands = CommandsDto(command_payload).get_command_responses()
    ...
    def configuration_handler(self, client: mqtt.Client, userdata, message):
        ...
        if configuration_payload:
            self.config = ConfigurationStatusResponseDto(
